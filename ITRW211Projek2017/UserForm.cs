...
namespace ITRW211Projek2017
{
    public partial class UserForm : Form
    {
        ...
        private List<Stock> userChart = new List<Stock>();
        private string InvoiceNumber = "";
        private Random myRandom = new Random();

        private double totalExcl = 0;
            ...
            if (dgProducts.SelectedRows.Count != 0)
            {
                ...
                else
                {
                    ...
                    if (result)
                    {
                        ...
                        {
                            if (stock.Product == row.Cells["Product"].Value.ToString() && Convert.ToInt32(row.Cells["Quantity"].Value) > 0 && Convert.ToInt32(row.Cells["Quantity"].Value) > stock.Quantity)
                            {
        ...
        private void btnBuy_Click(object sender, EventArgs e)
        {
            foreach (var stock in userChart)
            {
                var update = new OleDbCommand("UPDATE Stock  SET Quantity = Quantity-'" + stock.Quantity + "' WHERE ID = " + stock.Id + "", myDB);
            ...
            using (StreamWriter writer = new StreamWriter(Global.invoiceLocation + "\\Invoice-" + InvoiceNumber + ".txt"))
            {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            if (dgChart.SelectedRows.Count != 0)
            {
                ...
                var moreThanOneProduct = userChart.Any(product => product.Product == row.Cells["Product"].Value.ToString() && product.Quantity > 1);
